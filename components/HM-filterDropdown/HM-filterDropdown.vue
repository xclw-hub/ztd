			...
			<view class="sub-menu-class" :class="{'show':showPage==page_index,'hide':pageState[page_index]!=true}">
				...
				<block v-if="page.type=='filter'">
					<view class="filter">
						...
						<view class="btn-box">
							<view class="reset" @tap="resetFilterData(page_index)">取消</view>
				...
				<block v-if="page.type=='radio'">
					<view class="filter">
						<scroll-view class="row-menu-box" :scroll-y="true">
							<view class="box" v-for="(box,box_index) in page.submenu" :key="box_index">
								<view class="title">{{box.name}}</view>
								<view class="labels">
				...
				<block v-if="page.type=='editor'">
					<view class="editor-filter">
						<scroll-view class="menu-box" :scroll-y="true">
							<view class="box">
								<view class="title">价格区间（元）</view>
								<view class="labels">
									<input class="uni-input" @input="onKeyInput" placeholder="最低价" />
									<span style="margin: 12px 10px;">-</span>
									<input class="uni-input" @input="onKeyInput" placeholder="最高价" />
								</view>
								<view class="tips">注：最多可保留两位小数</view>
							</view>
						</scroll-view>
						<view class="btn-box">
							<view class="reset" @tap="resetFilterData(page_index)">取消</view>
